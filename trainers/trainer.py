...
class Trainer(object):
    def _load_best_for_eval(self, ckpt_path=None):
      """Load best checkpoint saved by EarlyStoppingNew for eval/test."""
      if ckpt_path is None:
          ckpt_path = os.path.join(self.args.output_dir, "pytorch_model.bin")
      if not os.path.exists(ckpt_path):
          self.logger.warning(f"[WARN] Best checkpoint not found: {ckpt_path}")
          return False
      checkpoint = torch.load(ckpt_path, map_location=self.device)
      state_dict = checkpoint.get("state_dict", checkpoint)
      self.model.load_state_dict(state_dict, strict=False)
      self.logger.info(f"[OK] Loaded best checkpoint from: {ckpt_path}")
      return True
    ...
    def train(self):
        ...
        self._load_best_for_eval()   # <= 이 한 줄
